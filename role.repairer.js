...
module.exports = {
    ...
    shouldCreate: function(spawner){
        return this.current(spawner).length < 2;//spawner.memory.minNumber[this.ROLE];
    },
    ...
    run: function (creep){
        if(creep.room.find(FIND_STRUCTURES, {filter: (s) => s.hits < s.hitsMax && s.structureType != STRUCTURE_WALL}).length == 0){
