...
module.exports = {
    ...
    selectLocation: function(creep) {
        ...
        let containers = creep.room.find(FIND_STRUCTURES, {
            filter: function(s) {
                return s.structureType == STRUCTURE_CONTAINER && _.some(sources, (source) => source.pos.inRangeTo(s, 1))
            }
        });
        // Get all the haulers that have targets
        let haulers = creep.room.find(FIND_MY_CREEPS, {
            filter: (c) => c.memory.role == this.ROLE && c.memory.target != null
        })
    ...
    run: function(creep) {
        if (creep.carry.energy > creep.carryCapacity / 2) {
            ...
            if (cont.store[RESOURCE_ENERGY] == 0)
                cont = creep.pos.findClosestByPath(FIND_STRUCTURES, {
                    filter: function(s) {
